...
const SITUACAO = (media: number | null) => {
  ...
  if (media >= 14) return { label: "Aprovado", className: "bg-green-100 text-green-700 dark:bg-green-500/20 dark:text-green-400" }
  if (media >= 10) return { label: "Recuperação", className: "bg-yellow-100 text-yellow-700 dark:bg-yellow-500/20 dark:text-yellow-400" }
...
export default function ProfessorPautasPage() {
  ...
  const aprovados = alunos.filter((a) => a.media !== null && Number(a.media) >= 14).length
  const recuperacao = alunos.filter((a) => a.media !== null && Number(a.media) >= 10 && Number(a.media) < 14).length
                    ...
                    <table className="w-full">
                      <thead>
                        <tr className="border-b border-border">
                          ...
                          <th className="text-center text-xs font-medium text-muted-foreground uppercase px-4 py-3">P1 (40%)</th>
                          <th className="text-center text-xs font-medium text-muted-foreground uppercase px-4 py-3">P2 (40%)</th>
                        ...
                        {paginados.length > 0 ? paginados.map((aluno, i) => {
                          ...
                          return (
                            <motion.tr key={aluno.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.03 }} className="border-b border-border last:border-0 hover:bg-muted/30 transition-colors">
                              ...
                              <td className="px-4 py-3.5 text-center text-sm">{aluno.prova_professor ?? "—"}</td>
                              <td className="px-4 py-3.5 text-center text-sm">{aluno.avaliacao ?? "—"}</td>
                              <td className="px-4 py-3.5 text-center text-sm">{aluno.prova_trimestre ?? "—"}</td>
