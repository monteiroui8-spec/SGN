...
function exportarCSV(notas: NotaPendente[], trimestre: string) {
  const header = "Aluno,Turma,Disciplina,Prova Prof,Avaliação,Prova Trim,Média,Estado\n"
  const rows = notas.map(n =>
    `"${n.aluno_nome}","${n.turma_nome || ''}","${n.disciplina_nome}",${n.prova_professor ?? ''},${n.avaliacao ?? ''},${n.prova_trimestre ?? ''},${n.media ?? ''},${n.media !== null && Number(n.media) >= 10 ? 'Aprovado' : 'Reprovado'}`
  ).join("\n")
...
function exportarPDF(notas: NotaPendente[], trimestre: string, mediaGeral: number, aprovadas: number) {
  const rows = notas.map((n, i) => `
    <tr style="background:${i % 2 === 0 ? '#fff' : '#f9fafb'}">
      ...
      <td style="padding:8px 6px;text-align:center;border-bottom:1px solid #e5e7eb;">${n.prova_professor ?? '—'}</td>
      <td style="padding:8px 6px;text-align:center;border-bottom:1px solid #e5e7eb;">${n.avaliacao ?? '—'}</td>
      <td style="padding:8px 6px;text-align:center;border-bottom:1px solid #e5e7eb;">${n.prova_trimestre ?? '—'}</td>
...
<table>
  <thead><tr>
    ...
    <th style="text-align:center">P.Prof</th><th style="text-align:center">Aval.</th>
    <th style="text-align:center">P.Trim</th><th style="text-align:center">Média</th>
  </tr></thead>
  <tbody>${rows}</tbody>
</table>
                    ...
                    <table className="w-full text-sm">
                      <thead>
                        <tr className="border-b bg-muted/30">
                          ...
                          <th className="text-center py-3 px-2 font-semibold">P.Prof</th>
                          <th className="text-center py-3 px-2 font-semibold">Aval.</th>
                          <th className="text-center py-3 px-2 font-semibold">P.Trim</th>
                      ...
                      <tbody>
                        {notas.map((n) => (
                          <tr key={n.id} className="border-b hover:bg-muted/20">
                            ...
                            <td className="py-2 px-2 text-center">{(n as any).prova_professor ?? "—"}</td>
                            <td className="py-2 px-2 text-center">{(n as any).avaliacao ?? "—"}</td>
                            <td className="py-2 px-2 text-center">{(n as any).prova_trimestre ?? "—"}</td>
