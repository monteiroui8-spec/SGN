 ...
 * Fórmula: (Prova Professor + Avaliação + Prova Trimestre) / 3
 */
class EmailTemplate {
    ...
    public static function boletim(array $aluno, array $notas, array $trimestre, array $encarregado): string {
        ...
        foreach ($notas as $i => $nota) {
            ...
            $pp  = $nota['prova_professor'] !== null ? number_format(floatval($nota['prova_professor']), 1, ',', '') : '—';
            $av  = $nota['avaliacao']       !== null ? number_format(floatval($nota['avaliacao']),       1, ',', '') : '—';
            $pt  = $nota['prova_trimestre'] !== null ? number_format(floatval($nota['prova_trimestre']), 1, ',', '') : '—';

            $feedbackHtml = '';
            if (!empty($nota['feedback'])) {
                $feedbackHtml = "<tr><td colspan='5' style='padding:6px 16px 12px;color:#6b7280;font-size:12px;font-style:italic;'>💬 " . htmlspecialchars($nota['feedback']) . "</td></tr>";
            }

            $linhasNotas .= "
            <tr style='background-color:{$bgRow};'>
              <td style='padding:12px 16px;border-bottom:1px solid #e5e7eb;font-weight:500;color:#111827;'>{$nota['disciplina_nome']}</td>
              <td style='padding:12px 8px;border-bottom:1px solid #e5e7eb;text-align:center;color:#374151;'>{$pp}</td>
              <td style='padding:12px 8px;border-bottom:1px solid #e5e7eb;text-align:center;color:#374151;'>{$av}</td>
              <td style='padding:12px 8px;border-bottom:1px solid #e5e7eb;text-align:center;color:#374151;'>{$pt}</td>
        ...
        <table style="width:100%;border-collapse:collapse;font-size:14px;">
          <thead>
            <tr style="background:#f8f9fa;">
              ...
              <th style="padding:12px 8px;text-align:center;border-bottom:2px solid #e5e7eb;font-size:12px;">P.Prof<br><span style="font-weight:400;color:#9ca3af;">÷3</span></th>
              <th style="padding:12px 8px;text-align:center;border-bottom:2px solid #e5e7eb;font-size:12px;">Aval.<br><span style="font-weight:400;color:#9ca3af;">÷3</span></th>
              <th style="padding:12px 8px;text-align:center;border-bottom:2px solid #e5e7eb;font-size:12px;">P.Trim.<br><span style="font-weight:400;color:#9ca3af;">÷3</span></th>
          ...
          <tfoot>
            <tr style="background:linear-gradient(135deg,#eff6ff,#f5f3ff);">
              <td colspan="4" style="padding:16px;text-align:right;font-weight:700;font-size:15px;">Média Geral:</td>
