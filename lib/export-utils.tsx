...
// Follows Angola's grading system: P1(25%) + P2(25%) + Trabalho(10%) + Exame(40%)

/**
 * Calculate final grade according to Angola's grading system
 */
export function calcularMediaAngola(p1: number, p2: number, trabalho: number, exame?: number | null): number | null {
  if (exame === null || exame === undefined) {
    // Without exam, calculate partial average
    const mediaParciall = (p1 * 0.25 + p2 * 0.25 + trabalho * 0.1) / 0.6
    return Math.round(mediaParciall * 10) / 10
  }
  const media = p1 * 0.25 + p2 * 0.25 + trabalho * 0.1 + exame * 0.4
    ...
    <table>
      <thead>
        <tr>
          <th>Disciplina</th>
          <th>Professor</th>
          <th class="text-center">P1 (25%)</th>
          <th class="text-center">P2 (25%)</th>
          <th class="text-center">Trab. (10%)</th>
